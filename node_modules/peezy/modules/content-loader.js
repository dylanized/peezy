	...
	ContentLoader.getPages = function(folderpath) {
		...
		for (var i = 0; i < page_files.length;) {
			...
			if (page_files[i].indexOf(".DS_Store") > -1) page_files.splice(i, 1);
				...
				// strip out custom template from filename
				if (path.extname(page.bare)) {
				
					var template_ext = path.extname(page.bare);
					console.log("Template " + template_ext);
					page.slug = path.basename(page.bare, template_ext);
				
				}
				else page.slug = page.bare;
				...
				// template override
				if (template_ext) page.template = template_ext.replace(".", "");
