...
// module init
	...
	peezy.init = function(app_config, cb) {
		...
		// load content
			...
			var themes = fileHelper.getFolders(paths.themes_abs);
			...
			function get_pages(folderpath) {
				...
				for (var i = 0; i < page_files.length; i++) {
					...
					if (page_files[i].indexOf(".DS_Store") > -1) page_files.splice(i, 1);
						...
						// remove folderpath
						page_files[i] = page_files[i].replace(folderpath, "");
					
						// remove file ext
						page_files[i] = page_files[i].replace(".html", "");
		...
		// functions
			...
			function renderTemplate(template, req, res) {
				...
				// setup
					...
					// if theme override
					if (req.query.theme && theme_exists(req.query.theme)) render_theme = req.query.theme;
				...
				// helpers				
					...
					function theme_exists(slug) {
						var config = path.join(paths.themes_abs, slug, "theme.json");
						var index = path.join(paths.themes_abs, slug, "index.ejs");
						if (fileHelper.exists(config) && fileHelper.exists(index)) return true;
						else return false;						
					}
