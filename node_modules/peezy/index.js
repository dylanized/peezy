...
// module init
	...
	peezy.init = function(app_config, cb) {
		...
		// express init
			...
			test_msg("Mode: " + app.get('env'));	
								
		// peezy init
		
			var pages = contentLoader.getPages(paths.contentBase);			
			test_msg(pages.length + " Pages Loaded: ", pages);
			
			themeEngine.init(app_config, site_config, app);			
			//test_msg(themeEngine.locals.themes.length + " Themes Loaded: ", themeEngine.locals.themes);
		...
		// functions
			...
			function handleReq(slug, req, res) {
				...
				themeEngine.setTemplate(page, req.query);
