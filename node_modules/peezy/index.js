...
// module init
	...
	peezy.init = function(app_config, cb) {
		...
		// load content and themes
			...
			themeEngine.init(app_config, site_config, pages);
		...
		// routing
			...
			router.get("/", function(req, res) {
				// render homepage
			   renderTheme(site_config.homepage, req, res);
			});
			...
			app.use(function(req, res){
				// render error page
				renderTheme(site_config.error, req, res);
			});				
		...
		// functions
			...
			function renderTheme(slug, req, res) {

				// check for 404
				if (!pages[slug]) slug = site_config.error;
		
				// set theme
				themeEngine.set(slug, pages[slug].content, req.query);					
				app.set("views", themeEngine.themeBase);							

				// render view
				res.status(themeEngine.status).render(themeEngine.templateFile, themeEngine.locals);
